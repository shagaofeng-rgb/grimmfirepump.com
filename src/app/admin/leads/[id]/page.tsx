...
export default async function LeadDetailPage({ params }: Props) {
  ...
  return <AdminShell>
    <Link href="/admin/leads" className="inline-flex items-center gap-2 text-sm font-black text-orange-700"><ArrowLeft size={16} /> 返回客户询盘</Link>
      <AdminPageHeader eyebrow="客户线索" title={lead.name || lead.email} />
    <div className="mt-7 grid gap-6 xl:grid-cols-[minmax(0,1fr)_380px]">
      <div className="grid gap-6">
        ...
        <AdminCard title="来源与提交信息"><dl className="grid gap-3 sm:grid-cols-2"><Value label="来源类型" value={lead.sourceType} /><Value label="来源页面" value={lead.sourcePage} /><Value label="访问渠道" value={lead.channel} /><Value label="外部来源" value={lead.referrer} /><Value label="营销来源" value={lead.utmSource} /><Value label="营销活动" value={lead.utmCampaign} /><Value label="提交时间" value={stamp(lead.createdAt)} /><Value label="隐私同意" value={lead.privacyConsent} />{lead.whatsappClickId ? <><Value label="WhatsApp 账号" value={lead.whatsappAccountLabel} /><Value label="WhatsApp 点击入口" value={lead.whatsappClickSource} /><Value label="WhatsApp 点击页面" value={lead.whatsappClickPage} /><Value label="WhatsApp 点击时间" value={lead.whatsappClickedAt ? stamp(lead.whatsappClickedAt) : undefined} /></> : null}</dl></AdminCard>
        <AdminCard title="访问轨迹"><div className="mb-4 flex items-center justify-between gap-3">{lead.visitorId ? <Link href={`/admin/analytics/visitors/${encodeURIComponent(lead.visitorId)}?range=all`} className="shrink-0 text-sm font-black text-orange-700">查看访问详情</Link> : null}</div>{sessions.length ? <div className="grid gap-3">{sessions.slice(0, 8).map((session) => <article key={session.sessionId} className="rounded-md border border-slate-200 p-4"><div className="flex flex-wrap items-center justify-between gap-2"><strong className="text-sm text-slate-900">{stamp(session.startedAt)}</strong><span className="text-xs text-slate-500">{session.country} · {session.channel} · {session.eventCount} 个行为</span></div><div className="mt-3 flex items-start gap-2 text-sm text-slate-700"><Route size={15} className="mt-0.5 shrink-0 text-orange-700" /><span className="break-words">{session.entryPath} → {session.exitPath}</span></div></article>)}</div> : <p className="rounded-md bg-slate-50 p-4 text-sm text-slate-500">暂无访问记录。</p>}</AdminCard>
