  ...
  return (
    <AdminShell>
      <AdminPageHeader
        ...
        description="兼容 Contact Us、产品询价、快速报价、下载资料、OEM/ODM 和第三方表单来源。支持状态、意向等级、负责人、备注和 CSV 导出。"
      ...
      <div className="mt-8 overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
        <div className="grid gap-3 border-b border-slate-200 p-4 md:grid-cols-[1fr_180px_180px]">
          ...
          <select className={inputClass}><option>全部来源</option><option>Contact Us</option><option>Product Inquiry</option><option>Download</option><option>Facebook</option></select>
              ...
              <div className="grid gap-4 lg:grid-cols-[1fr_360px]">
                <div>
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    <span className="rounded-full bg-orange-50 px-2 py-1 text-xs font-black text-orange-700">Score {lead.score}</span>
                    <span className="rounded-full bg-slate-100 px-2 py-1 text-xs font-black text-slate-600">{lead.status || lead.stage || "new"}</span>
                  </div>
                  <p className="mt-2 text-sm text-slate-500">{lead.email} · {lead.phone || "No phone"} · {lead.country || "Country TBD"} · {lead.company || "Company TBD"}</p>
                  <p className="mt-3 text-sm leading-6 text-slate-700">{lead.product || "Product TBD"} / Flow: {lead.flow || "-"} / Head: {lead.head || "-"} / Cert: {lead.certification || "-"}</p>
                  <p className="mt-3 rounded-md bg-slate-50 p-3 text-sm leading-6 text-slate-600">{lead.message || "No message"}</p>
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2">
          {downloadLeads.map((lead) => (
            <div key={lead.id} className="rounded-md bg-slate-50 p-4 text-sm">
              ...
              <p className="mt-1 text-slate-500">{lead.assetTitle} · {lead.email} · {lead.country || "Country TBD"}</p>
            </div>
          ))}
        </div>
      </section>
