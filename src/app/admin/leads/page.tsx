  ...
  return <AdminShell>
    ...
    <section className="mt-8 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
      ...
      <div className="overflow-x-auto"><table className="min-w-[1040px] w-full text-left text-sm"><thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500"><tr><th className="px-5 py-3">客户</th><th className="px-5 py-3">需求</th><th className="px-5 py-3">来源</th><th className="px-5 py-3">意向 / 状态</th><th className="px-5 py-3">负责人</th><th className="px-5 py-3">提交时间</th><th className="px-5 py-3"></th></tr></thead><tbody className="divide-y divide-slate-100">
        {paged.items.map((lead) => <tr key={lead.id} className="hover:bg-slate-50/70"><td className="px-5 py-4"><strong className="block text-slate-950">{lead.name}</strong><span className="block max-w-[220px] truncate text-xs text-slate-500">{lead.company || "未填写公司"} · {lead.country || "未填写国家"}</span><span className="block max-w-[220px] truncate text-xs text-slate-500">{lead.email}</span></td><td className="px-5 py-4"><strong className="block max-w-[200px] truncate text-slate-800">{lead.product || "一般咨询"}</strong><span className="block text-xs text-slate-500">{lead.flow || "-"} / {lead.head || "-"}</span></td><td className="px-5 py-4"><span className="block font-bold text-slate-700">{lead.sourceType || "网站表单"}</span><span className="block max-w-[180px] truncate text-xs text-slate-500">{lead.sourcePage || "/"}</span>{lead.whatsappClickId ? <span className="mt-1 inline-flex rounded-full bg-emerald-50 px-2 py-0.5 text-[11px] font-black text-emerald-700">已归因 WhatsApp</span> : null}</td><td className="px-5 py-4"><div className="flex flex-wrap gap-1.5"><StatusPill value={lead.status || lead.stage || "new"} />{lead.intent ? <span className="rounded-full bg-orange-50 px-2.5 py-1 text-xs font-black text-orange-700">{lead.intent} 类</span> : null}</div><span className="mt-1 block text-xs text-slate-500">评分 {lead.score}</span></td><td className="px-5 py-4 text-slate-600">{lead.owner || "未分配"}</td><td className="px-5 py-4 text-slate-600">{stamp(lead.createdAt)}</td><td className="px-5 py-4"><Link href={adminQuery(`/admin/leads/${lead.id}`, { range: range.preset, from: range.from, to: range.to })} className="relative z-20 inline-flex min-h-10 min-w-[76px] items-center justify-center whitespace-nowrap rounded-md border border-[#071426] bg-[#071426] px-3 py-2 text-xs font-black text-white opacity-100 shadow-sm transition-colors hover:bg-[#123b63] hover:text-white">查看详情</Link></td></tr>)}
