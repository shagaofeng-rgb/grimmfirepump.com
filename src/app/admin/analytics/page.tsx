...
export default async function AdminAnalyticsPage({ searchParams }: PageProps) {
  ...
  return (
    <AdminShell>
      <AdminPageHeader eyebrow="真实流量分析" title="访客、来源和转化运营中心" description="默认隐藏预览、Collects/自动化与已配置测试流量。按统一时间范围查看真实访问、客户归属与路径。" action={<AnalyticsRefresh />} />

      <section className="mt-8 overflow-hidden rounded-xl bg-[#091b32] p-5 text-white shadow-[0_20px_60px_rgba(15,23,42,0.18)] md:p-6">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between"><div><p className="text-xs font-black uppercase tracking-[0.18em] text-orange-300">Traffic intelligence</p><h2 className="mt-2 text-2xl font-black">统一筛选真实经营数据</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-300">时间、国家、来源渠道和流量类型同时作用于指标、客户档案、事件与导出。</p></div><div className="rounded-md border border-white/10 bg-white/5 px-4 py-3 text-sm"><span className="text-slate-300">已过滤非真实事件</span><strong className="ml-2 text-lg text-orange-300">{summary.filteredEvents}</strong></div></div>
        <form className="mt-6 grid gap-3 xl:grid-cols-[1.3fr_repeat(4,minmax(0,1fr))]" method="get">
          ...
          <div className="flex gap-2"><input name="query" defaultValue={filters.query} placeholder="页面、访客、IP、来源..." className="min-h-11 min-w-0 flex-1 rounded-md border border-white/15 bg-white px-3 text-sm text-slate-900" /><button className="button button-primary min-h-11" type="submit">应用筛选</button></div>
        </form>
      </section>
      ...
      <section className="mt-8 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="flex items-center justify-between border-t border-slate-200 px-5 py-4 text-sm text-slate-600"><span>第 {pagedProfiles.page} / {pagedProfiles.totalPages} 页，共 {pagedProfiles.total} 个访客</span><div className="flex gap-2"><Link aria-disabled={pagedProfiles.page <= 1} className="rounded-md border border-slate-200 px-3 py-2 font-bold aria-disabled:pointer-events-none aria-disabled:opacity-40" href={queryFor({ ...baseQuery, visitorPage: pagedProfiles.page - 1 })}>上一页</Link><Link aria-disabled={pagedProfiles.page >= pagedProfiles.totalPages} className="rounded-md border border-slate-200 px-3 py-2 font-bold aria-disabled:pointer-events-none aria-disabled:opacity-40" href={queryFor({ ...baseQuery, visitorPage: pagedProfiles.page + 1 })}>下一页</Link></div></div>
      </section>
      ...
      <section className="mt-8 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="flex items-center justify-between border-t border-slate-200 px-5 py-4 text-sm text-slate-600"><span>第 {pagedEvents.page} / {pagedEvents.totalPages} 页，共 {pagedEvents.total} 条</span><div className="flex gap-2"><Link aria-disabled={pagedEvents.page <= 1} className="rounded-md border border-slate-200 px-3 py-2 font-bold aria-disabled:pointer-events-none aria-disabled:opacity-40" href={queryFor({ ...baseQuery, page: pagedEvents.page - 1 })}>上一页</Link><Link aria-disabled={pagedEvents.page >= pagedEvents.totalPages} className="rounded-md border border-slate-200 px-3 py-2 font-bold aria-disabled:pointer-events-none aria-disabled:opacity-40" href={queryFor({ ...baseQuery, page: pagedEvents.page + 1 })}>下一页</Link></div></div>
      </section>
    </AdminShell>
  );
}
