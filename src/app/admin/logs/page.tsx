...
export default async function LogsPage({ searchParams }: Props) {
  ...
  const filters = {
    ...
    result: value(params, "result") || "all",
  };
    ...
    <AdminShell>
      ...
      <section className="mt-8 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        <nav className="flex flex-wrap gap-2 border-b border-slate-200 bg-slate-50 p-3" aria-label="网站记录分类">
          <Link
            href={adminQuery("/admin/logs", { ...tabQuery, view: "activity" })}
            aria-current={view === "activity" ? "page" : undefined}
            className="inline-flex min-h-10 items-center justify-center rounded-md border border-slate-300 bg-white px-4 text-sm font-black text-slate-700 shadow-sm transition-colors hover:border-[#123b63] hover:text-[#123b63] aria-[current=page]:border-[#071426] aria-[current=page]:bg-[#071426] aria-[current=page]:text-white"
          >
            操作记录
          </Link>
          <Link
            href={adminQuery("/admin/logs", { ...tabQuery, view: "login" })}
        ...
        <form method="get" className="grid gap-3 border-b border-slate-200 bg-slate-50 p-4 md:grid-cols-4">
          <input type="hidden" name="view" value={view} />
          <div className="md:col-span-4">
            <DateRangeFilter pathname="/admin/logs" query={filters} preset={range.preset} from={range.from} to={range.to} />
          </div>
          <input name="query" defaultValue={filters.query} className={inputClass} placeholder={view === "login" ? "搜索账号或登录原因" : "搜索操作人、动作或对象"} />
          <select name="result" defaultValue={filters.result} className={inputClass}>
            <option value="all">全部结果</option>
            <option value="success">成功</option>
            <option value="failed">失败</option>
          </select>
          <select name="pageSize" defaultValue={String(pagination.pageSize)} className={inputClass}>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-[760px] w-full text-left text-sm">
            <thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500">
              <tr>
                {(view === "login" ? ["账号", "结果", "原因", "时间"] : ["操作人", "操作", "对象", "结果", "时间"]).map((head) => (
                  <th key={head} className="px-5 py-3 font-black">{head}</th>
                ))}
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {view === "login"
                ? pagedLogins.items.map((item) => (
                  <tr key={item.id}>
                    <td className="px-5 py-4 font-bold text-slate-900">{item.username}</td>
                    <td className="px-5 py-4"><StatusPill value={item.success ? "success" : "failed"} /></td>
                    <td className="px-5 py-4 text-slate-700">{item.reason}</td>
                    <td className="px-5 py-4 text-slate-600">{new Date(item.createdAt).toLocaleString("zh-CN")}</td>
                  </tr>
                ))
                : pagedActivity.items.map((item) => (
                  <tr key={item.id}>
                    <td className="px-5 py-4 font-bold text-slate-900">{item.actor}</td>
                    <td className="px-5 py-4 text-slate-700">{item.action}</td>
                    <td className="px-5 py-4 text-slate-700">{item.target}</td>
                    <td className="px-5 py-4"><StatusPill value={item.result} /></td>
                    <td className="px-5 py-4 text-slate-600">{new Date(item.createdAt).toLocaleString("zh-CN")}</td>
                  </tr>
                ))}
            </tbody>
          </table>
          {!pagination.items.length ? <div className="p-5"><EmptyState text="当前筛选条件下没有记录。" /></div> : null}
        </div>
        <AdminPagination pathname="/admin/logs" query={{ ...filters, pageSize: pagination.pageSize }} page={pagination.page} totalPages={pagination.totalPages} total={pagination.total} pageSize={pagination.pageSize} label="记录" />
      </section>
    </AdminShell>
  );
}
