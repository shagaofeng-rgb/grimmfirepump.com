...
export default async function MediaPage() {
  ...
  return (
    <AdminShell>
      <AdminPageHeader eyebrow="媒体资源" title="图片、PDF、视频和技术文件资源库" description="当前版本支持安全登记已有文件路径和外部资源 URL；二进制上传已预留，建议接入 Vercel Blob、S3 或 Cloudflare R2。" />
      <div className="mt-8 grid gap-6 xl:grid-cols-[420px_1fr]">
        ...
        <section className="grid gap-4">
          {media.map((item) => (
            <article key={item.id} className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
              <div className="flex flex-wrap justify-between gap-4">
                <div><strong>{item.name}</strong><p className="mt-1 text-sm text-slate-500">{item.type} · {item.folder} · {item.sizeLabel || "size unknown"}</p><p className="mt-2 break-all text-sm text-slate-600">{item.url}</p></div>
          ...
          {!media.length ? <EmptyState text="暂无媒体资源。请先登记现有 public/assets 或外部存储文件。" /> : null}
        </section>
      </div>
    </AdminShell>
  );
